...
const sendSlicedRequesters = async (slicedRequesters: Requester[], tries: number, counter: number): Promise<boolean> => {
  if (counter >= tries) {
  ...
  if (!response) {
    ...
    return sendSlicedRequesters(slicedRequesters, tries, counter++)
  }
