...
const REDE_INDIVIDUAL_GEOLOCATION_MUTATION = gql`
  mutation update_rede_individuals(
    $id: Int!
    $address: String!
    $state: String!
    $city: String!
...
export const validateMutationRes = async (schema, updatedIndividual) => {
  try {
    const validation = await schema.validate(updatedIndividual, {
      strict: true,
      abortEarly: false
    });
    logger.log(
      "info",
      "successfuly validated schema of updated coordinates mutation"
    );
    return validation;
  } catch (e) {
...
export const mutationUpdateCoordinates = async (
  individual: IndividualGeolocation
) => {
  try {
    const {
      data: {
        update_rede_individuals: { returning: updatedIndividual }
      }
    ...
    logger.log(
      "info",
      `Updated individual "${individual.id}" coordinates in Hasura`
    );

    return updatedIndividual;
  } catch (err) {
    logger.error(
      `Failed to update individual "${individual.id}" coordinates in Hasura `,
      err
    );
    throw new Error("Failed to update individual coordinates in Hasura");
  }
};
...
export const geolocation = (response: SubscribeIndividualsResponse) => {
  const {
    data: { rede_individuals: individuals }
  } = response;

  individuals.forEach(async (individual: SubscribeIndividual) => {
    const individualWithGeolocation = await convertCepToAddressWithGoogleApi(
      individual
    );
    ...
    const validateDataForMutation = await validateMutationRes(
      schema,
      individualWithGeolocation
    );

    if (!validateDataForMutation) {
      throw new Error("Updated coordinates failed validation");
    }

    type UpdateCoordinatesRes = yup.InferType<typeof schema>;

    const updatedIndividual: UpdateCoordinatesRes = await mutationUpdateCoordinates(
      individualWithGeolocation
    );
...
export const subscriptionRedesIndividuals = async (): Promise<ZenObservable.Subscription> => {
  try {
    ...
    throw new Error("Failed on fetching subscription");
  }
};
